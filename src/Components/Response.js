...
function Response(props) {
 ...
 // *** INITIAL FETCH FOR AN ARRAY OF HOURS 
  ...
  let sunsetHour =  Number(w.forecast.forecastday[0].astro.sunset.slice(0, 2)) + 12; // finds and converts sunset
...
// **FINDS START TIME ***
  ...
  let sortedComfortableTemp = sortTemps(comfortableTemp, "humidity", "low-high");
  ...
  let sortedRainyComfortable = sortTemps(rainyComfortable, "totalprecip_mm", "low-high")
  console.log("Sorted temps", sortedComfortableTemp, sortedVeryHot, sortedCold, sortedRainyCold, sortedRainyHot, sortedRainyComfortable)
  ...
  function findTime(arrayToCheck) {
            ...
            weatherConditionsAtTime = "Weather conditions at time: " + arrayToCheck[0][1].condition.text.toLowerCase();
    }
  ...
  if (sortedComfortableTemp.length > 0) {
    ...
    } else if (sortedRainyComfortable > 0) {
      findTime(sortedRainyComfortable)
    } else if (sortedRainyHot > 0) {
      findTime(sortedRainyHot) 
    } else if (sortedRainyCold > 0) {
  ...
  return (
    <div className="Response">
      <p>
        {" "}
        The optimum time for your walk is:<br></br><span id="time"> {optimumTime} </span>
       <br></br> {lateMessage}
      </p>
